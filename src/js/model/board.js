...
export class Board {
  ...
  createBoardSpaces() {
    ...
    this.spacesArray = totalSpaces;
  }
}

const Heather = new Board(10, 10, 5, 5);
Heather.createBoardSpaces();
Heather.spacesArray.forEach((space) => {
  if (space.special !== null) {
    console.log(
      `value = ${space.value}, type = ${space.type} special = ${space.special.value}`
    );
  } else {
    console.log(`value = ${space.value}, type = ${space.type}`);
  }
});
